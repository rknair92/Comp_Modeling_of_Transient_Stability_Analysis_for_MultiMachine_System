...
% This program may take 5 minutes or more to run
clc
clear
%% Reading original bus data
bdata.busdata='busdata6';
bdata=busdata6(bdata);
...
% if angle is not sepcified, assign 0
% if Pgen  is not sepcified, assign 0
% if Qgen  is not sepcified, assign
% if Pload is not sepcified, assign 0
Ymag=abs(Y);
Yang=angle(Y);
...
% Mismatch of P
 %bust(p)=0,3 for PQ bus, bust(p)=2 for PV bus,bust(p)=3,1 for slack bus   
for p = 1:n
...
genConnection=bdata.genConn;
for ncount=1:length
loadnf=ncount;
nf=ncount;   %faulty node
node1=nf;  %fault cleared by opening node 1 & node 2
node2=bdata.connection(nf);
...
count1=0;
%removing row and column of the faulty node in Y4 
for p=1:m+n
    if p~=nf+m
    count1=count1+1;
    count2=0;
...
%  fault cleared by opening line between node1 and node2 in the origianl system
...
if any(abs(loadnf-genConnection)<1e-10)

diffDel=((del(:,2)-del(:,1))*180/pi);
busdel(1)=max(diffDel(diffDel<max(diffDel)));
diffDel=((del(:,3)-del(:,1))*180/pi);
busdel(2)=max(diffDel(diffDel<max(diffDel)));
diffDel=((del(:,4)-del(:,1))*180/pi);
busdel(3)=max(diffDel(diffDel<max(diffDel)));
diffDel=((del(:,5)-del(:,1))*180/pi);
busdel(4)=max(diffDel(diffDel<max(diffDel)));
diffDel=((del(:,6)-del(:,1))*180/pi);
busdel(5)=max(diffDel(diffDel<max(diffDel)));
diffDel=((del(:,7)-del(:,1))*180/pi);
busdel(6)=max(diffDel(diffDel<max(diffDel)));
diffDel=((del(:,8)-del(:,1))*180/pi);
busdel(7)=max(diffDel(diffDel<max(diffDel)));
diffDel=((del(:,9)-del(:,1))*180/pi);
busdel(8)=max(diffDel(diffDel<max(diffDel)));
diffDel=((del(:,10)-del(:,1))*180/pi);
busdel(9)=max(diffDel(diffDel<max(diffDel)));

if min(abs(busdel))>150
    flag=0;
    critcaltime(loadnf)=0.01*inctime-0.01
end

else
busdel(1)=max((del(:,2)-del(:,1))*180/pi);
busdel(2)=max((del(:,3)-del(:,1))*180/pi);
busdel(3)=max((del(:,4)-del(:,1))*180/pi);
busdel(4)=max((del(:,5)-del(:,1))*180/pi);
busdel(5)=max((del(:,6)-del(:,1))*180/pi);
busdel(6)=max((del(:,7)-del(:,1))*180/pi);
busdel(7)=max((del(:,8)-del(:,1))*180/pi);
busdel(8)=max((del(:,9)-del(:,1))*180/pi);
busdel(9)=max((del(:,10)-del(:,1))*180/pi);

if max(abs(busdel))>150
    flag=0;
    critcaltime(loadnf)=(0.01*inctime)-0.01
end
